...
import { FileText, Shield, AlertTriangle, CheckCircle, TrendingUp, FileCheck, Clock } from "lucide-react";
          ...
          <div className="space-y-3">
            {stats?.bySeverity?.map((s: any) => (
              <div key={s.severity} className="flex items-center gap-3">
                <span className={`text-xs font-bold uppercase w-20 ${s.severity === "minor" ? "text-green-700" : s.severity === "moderate" ? "text-amber-700" : s.severity === "serious" ? "text-orange-700" : "text-red-700"}`}>{s.severity}</span>
                <div className="flex-1 bg-gray-100 rounded-full h-6 overflow-hidden">
                  <div className={`h-full rounded-full flex items-center justify-end pr-2 text-[10px] font-bold text-white ${s.severity === "minor" ? "bg-green-500" : s.severity === "moderate" ? "bg-amber-500" : s.severity === "serious" ? "bg-orange-500" : "bg-red-500"}`} style={{ width: `${Math.min(100, (s.count / (totalReports || 1)) * 100 * 3)}%` }}>{s.count}</div>
                </div>
              </div>
            )) || <p className="text-sm text-gray-400">No data — create officers and submit reports</p>}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {stats?.byStatus?.map((s: any) => (
              <div key={s.status} className="flex items-center gap-3">
                ...
                <div className="flex-1 bg-gray-100 rounded-full h-6 overflow-hidden">
                  <div className={`h-full rounded-full flex items-center justify-end pr-2 text-[10px] font-bold text-white ${s.status === "approved" ? "bg-green-500" : s.status === "submitted" ? "bg-blue-500" : s.status === "under_review" ? "bg-amber-500" : "bg-gray-400"}`} style={{ width: `${Math.min(100, (s.count / (totalReports || 1)) * 100 * 3)}%` }}>{s.count}</div>
                </div>
              </div>
            )) || <p className="text-sm text-gray-400">No data — create officers and submit reports</p>}
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        {recentReports.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-gray-400 mt-1">Officers will submit reports from the mobile app</p>
          </div>
        ) : (
          <div className="space-y-3">
              ...
              return (
                <div key={report.id} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 truncate">{report.reportId} — <span className="text-gray-600">{report.location}</span></p>
                    <p className="text-xs text-gray-500">{report.severity} • {report.weather} • {report.accidentDate}</p>
                  </div>
                  <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${report.status === "approved" ? "bg-green-100 text-green-700" : report.status === "submitted" ? "bg-blue-100 text-blue-700" : report.status === "under_review" ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-700"}`}>{report.status.replace("_", " ")}</span>
                </div>
              );
            })}
          </div>
